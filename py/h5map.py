...
for ii in range(nxpanel):
    ...
    for jj in range(nypanel):
        ...
        for ll in range(mass.size):
            ...
            # if xi >= 0 and xi < nx and zj >= 0 and zj < ny:
            #     ZZ[xi][zj] += mass[ll]
            ...
            for mm in range(max(0, xi - Nsmooth), min(nx - 1, xi + Nsmooth)):
                for nn in range(max(0, zj - Nsmooth), min(ny - 1, zj + Nsmooth)):
                    ZZ[mm][nn] += mass[ll] * psf_x[mm - xi - Nsmooth] * psf_y[nn - zj - Nsmooth]

        ZZ = np.maximum(ZZ, fmin)
        ZZ = np.minimum(ZZ, fmax)

        # plot the data
        # img = ax[idx].imshow(ZZ.T, extent = [xmin, xmax, zmin, zmax], origin='lower', interpolation='none', vmin = fmin, vmax = fmax, cmap = "jet")
        # img = ax[idx].imshow(ZZ.T, extent = [xmin, xmax, zmin, zmax], origin='lower', interpolation='none', norm = LogNorm(vmin = fmin, vmax = fmax), cmap = "jet")
        # img = ax[idx].imshow(ZZ.T, extent = [xmin, xmax, zmin, zmax], origin='lower', interpolation='none', norm = LogNorm(vmin = fmin, vmax = fmax), cmap = cmap0)
        # img = ax[idx].imshow(ZZ.T, extent = [xmin, xmax, zmin, zmax], origin='lower', interpolation='none', norm = LogNorm(vmin = fmin, vmax = fmax), cmap = cmap1)
        # img = ax[idx].imshow(ZZ.T, extent = [xmin, xmax, zmin, zmax], origin='lower', interpolation='none', norm = LogNorm(vmin = fmin, vmax = fmax), cmap = cmap2)
        img = ax[idx].imshow(ZZ.T, extent = [xmin, xmax, zmin, zmax], origin='lower', interpolation='none', norm = LogNorm(vmin = fmin, vmax = fmax), cmap = cmap3)
        # img = ax[idx].imshow(ZZ.T, extent = [xmin, xmax, zmin, zmax], origin='lower', interpolation='none', norm = LogNorm(vmin = fmin, vmax = fmax), cmap = cmap4)
        # img = ax[idx].imshow(ZZ.T, extent = [xmin, xmax, zmin, zmax], origin='lower', interpolation='none', norm = LogNorm(vmin = fmin, vmax = fmax), cmap = cmap5)

        # overlay contour
        # ax[idx].contour(XX, YY, ZZ.T, 4, colors = "black", norm = LogNorm(vmin = fmin, vmax = fmax))
        # cont = ax[idx].contour(XX, YY, ZZ.T, levels = [3.1e+5, 1.0e+6, 3.1e+6, 1.0e+7], colors = "black", norm = LogNorm(vmin = fmin, vmax = fmax))
        # cont = ax[idx].contour(XX, YY, ZZ.T, levels = [3.1e+5, 1.0e+6, 3.1e+6, 1.0e+7, 3.1e+7], colors = "black", norm = LogNorm(vmin = fmin, vmax = fmax), linewidths = 0.7)
        cont = ax[idx].contour(XX, YY, ZZ.T, levels = [3.1e+5, 1.0e+6, 3.1e+6, 1.0e+7], colors = "black", norm = LogNorm(vmin = fmin, vmax = fmax), linewidths = 0.7)
        ...
        # ax[idx].tick_params(axis = "both", direction = "inout", bottom = "on", top = "on", left = "on", right = "on")
        ax[idx].tick_params(axis = "both", direction = "in", color = "white", bottom = "on", top = "on", left = "on", right = "on")
        # if ii == 0:
        #     ax[idx].tick_params(axis = "y", direction = "out", color = "black", left = "on")
        # if jj == 0:
        #     ax[idx].tick_params(axis = "x", direction = "out", color = "black", bottom = "on")
...
# finish benchmark
print("Nsmooth = {:d}, elapsed time is {:e} sec.".format(Nsmooth, time.clock() - start))
