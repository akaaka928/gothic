 ...
 double **spline_xx, double **spline_ff, double **spline_f2, double **spline_bp)
{
  ...
  *encSph = (double *)malloc(NDISKBIN_RAD * sizeof(double));  if( *encSph == NULL ){    __KILL__(stderr, "ERROR: failure to allocate encSph");  }
  //-----------------------------------------------------------------------
  /* for spline fitting */
  *spline_xx  = (double *)malloc((2 + ((*maxLev) + 1) * (NDISKBIN_HOR >> 1))                    * sizeof(double));
  *spline_ff  = (double *)malloc((2 + ((*maxLev) + 1) * (NDISKBIN_HOR >> 1)) * CPUS_PER_PROCESS * sizeof(double));
  *spline_f2  = (double *)malloc((2 + ((*maxLev) + 1) * (NDISKBIN_HOR >> 1)) * CPUS_PER_PROCESS * sizeof(double));
  *spline_bp  = (double *)malloc((2 + ((*maxLev) + 1) * (NDISKBIN_HOR >> 1)) * CPUS_PER_PROCESS * sizeof(double));
...
#pragma omp parallel for
	for(int ii = 0; ii < NDISKBIN_HOR; ii++){
	  ...
	  for(int jj = 1 + (NDISKBIN_VER >> 1); jj < NDISKBIN_VER - 1; jj++)
	    mass += (double)(1 << (1 + (jj & 1))) * (*disk[kk].rho)[INDEX(maxLev, NDISKBIN_HOR, NDISKBIN_VER, ll, ii, jj)];
...
#pragma omp critical
	...
	for(int jj = 1; jj < NDISKBIN_VER - 1; jj++)
	  Sigma += (double)(1 << (1 + (jj & 1))) * (*disk[kk].rhoSum)[INDEX(maxLev, NDISKBIN_HOR, NDISKBIN_VER, lev, ii, jj)];
...
#pragma omp critical
	...
	//-----------------------------------------------------------------
      ...
      /* for(int jj = 0; jj < NDISKBIN_VER; jj++) */
      /* 	(*disk[kk].rhoSum)[INDEX(maxLev, NDISKBIN_HOR, NDISKBIN_VER, lev, NDISKBIN_HOR - 1, jj)] = 0.0; */
      /* disk[kk].Sigma[INDEX2D(maxLev, NDISKBIN_HOR, lev, NDISKBIN_HOR - 1)] = 0.0; */
      //-------------------------------------------------------------------
    }/* for(int kk = 0; kk < ndisk; kk++){ */
    //---------------------------------------------------------------------
...
#pragma omp parallel for
  ...
  for(int kk = 0; kk < ndisk; kk++)
    for(int ll = maxLev - 2; ll >= 0; ll--){
      //-------------------------------------------------------------------
      coarseGraining(  disk[kk].pot , ll + 1, ll, &(disk[kk].hor[INDEX2D(maxLev, NDISKBIN_HOR, ll, 0)]), ldexp(disk[kk].hh, -ll));
      coarseGraining(*(disk[kk].rho), ll + 1, ll, &(disk[kk].hor[INDEX2D(maxLev, NDISKBIN_HOR, ll, 0)]), ldexp(disk[kk].hh, -ll));
      //-------------------------------------------------------------------
    }/* for(int ll = maxLev - 2; ll >= 0; ll--){ */
  //-----------------------------------------------------------------------
...
#pragma omp parallel for
  ...
  for(int kk = 0; kk < ndisk; kk++){
    for(int ll = maxLev - 1; ll >= 0; ll--){
      ...
      xx[0] = 0.0;
      for(int jj = 0; jj < NDISKBIN_VER; jj++)
	xx[1 + jj] = disk[kk].ver[INDEX2D(maxLev, NDISKBIN_VER, ll, jj)];
      xx[NDISKBIN_VER + 1] = xx[NDISKBIN_VER] + 0.5 * ldexp(disk[kk].hh, -ll);
      //-------------------------------------------------------------------
      const double z02 = xx[1] * xx[1];
      const double z12 = xx[2] * xx[2];
...
#pragma omp parallel for num_threads(CPUS_PER_PROCESS)
	...
	//-----------------------------------------------------------------
	ff[NDISKBIN_VER + 1] = (*disk[kk].rho)[INDEX(maxLev, NDISKBIN_HOR, NDISKBIN_VER, ll, ii, NDISKBIN_VER - 1)];
	for(int jj = NDISKBIN_VER - 1; jj >= 0; jj--)
	  ff[1 + jj] = (*disk[kk].rho)[INDEX(maxLev, NDISKBIN_HOR, NDISKBIN_VER, ll, ii, jj)];
	ff[0] = ((*disk[kk].rho)[INDEX(maxLev, NDISKBIN_HOR, NDISKBIN_VER, ll, ii, 0)] * z12 - (*disk[kk].rho)[INDEX(maxLev, NDISKBIN_HOR, NDISKBIN_VER, ll, ii, 1)] * z02) * zd2inv;
	//-----------------------------------------------------------------
    ...
    //---------------------------------------------------------------------
    for(int ii = 0; ii < NDISKBIN_HOR; ii++)
      disk[kk].sigmaz[ii] = 1.0 / (DBL_MIN + (*disk[kk].rhoSum)[INDEX(maxLev, NDISKBIN_HOR, NDISKBIN_VER, 0, ii, NDISKBIN_VER - 1)]);
    for(int ll = 0; ll < maxLev; ll++)
      for(int ii = 0; ii < NDISKBIN_HOR; ii++)
	for(int jj = 0; jj < NDISKBIN_VER; jj++)
