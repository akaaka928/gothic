/*************************************************************************\
 *                                                                       *
                  last updated on 2016/11/05(Sat) 15:38:41
 *                                                                       *
...
(const int numOld, const int numMax, int *numNew,  iparticle ibody_dev, iparticle src, iparticle dst,
 ...
 double *exchangeInterval, loadImbalanceDetector balancer[], MPIinfo orb[], domainCfg *domain, MPIcfg_tree mpi
...
)
{
  ...
  if( balancer[0].execute )    qsort(&sampleLoc[0               ], sampleNum, sizeof(real), posAscendingOrder);
  if( balancer[1].execute )    qsort(&sampleLoc[    sampleNumMax], sampleNum, sizeof(real), posAscendingOrder);
  if( balancer[2].execute )    qsort(&sampleLoc[2 * sampleNumMax], sampleNum, sizeof(real), posAscendingOrder);
  //-----------------------------------------------------------------------
  __NOTE__("rank %d\n", mpi.rank);
  //-----------------------------------------------------------------------
...
#pragma unroll
  ...
  for(int ii = NDIM - 1; ii >= 0; ii--){
    //---------------------------------------------------------------------
    if( balancer[ii].execute ){
      //-------------------------------------------------------------------
      /* gather sampling points to the root process */
      //-------------------------------------------------------------------
      /* set receive counts */
      const int nprocs = orb[ii].size;
      chkMPIerr(MPI_Gather(&sampleNum, 1, MPI_INT, recvNum, 1, MPI_INT, 0, orb[ii].comm));
      //-------------------------------------------------------------------
	...
	recvDsp[jj] = recvDsp[jj - 1] + recvNum[jj - 1];
      ...
      chkMPIerr(MPI_Gatherv(&sampleLoc[ii * sampleNumMax], sampleNum, MPI_REALDAT, sampleFul, recvNum, recvDsp, MPI_REALDAT, 0, orb[ii].comm));
      //-------------------------------------------------------------------
      /* sort sample particles in each direction */
      if( orb[ii].rank == 0 )
	...
	//-----------------------------------------------------------------
      ...
      chkMPIerr(MPI_Bcast(boxMin, mpi.dim[ii], MPI_REALDAT, 0, orb[ii].comm));
      chkMPIerr(MPI_Bcast(boxMax, mpi.dim[ii], MPI_REALDAT, 0, orb[ii].comm));
      //-------------------------------------------------------------------
  ...
  for(int ii = 0; ii < NDIM; ii++)
    if( balancer[ii].execute ){
      root = orb[ii];
      chkMPIerr(MPI_Allgather(&local, 1 * sizeof(domainCfg), MPI_BYTE, domain, 1 * sizeof(domainCfg), MPI_BYTE, root.comm));
      break;
    }/* if( balancer[ii].execute ){ */
  //-----------------------------------------------------------------------
  ...
  const int numProcs = root.size;
  //-----------------------------------------------------------------------
  for(int ii = 0; ii < numProcs; ii++)
    chkMPIerr(MPI_Irecv(&(recvBuf[ii].num), 1, MPI_INT, ii, ii, root.comm, &(recvBuf[ii].req)));
  //-----------------------------------------------------------------------
  ...
  for(int ii = 0; ii < numProcs; ii++){
	...
	(min[2] <= domain[ii].max[2]) && (max[2] >= domain[ii].min[2]) ){
      ...
      //-------------------------------------------------------------------
      sendBuf[overlapNum].xmin = (domain[ii].min[0] < QUARTER * REAL_MIN) ? (domain[ii].min[0]) : ((min[0] > domain[ii].min[0]) ? (min[0]) : (domain[ii].min[0]));
      sendBuf[overlapNum].ymin = (domain[ii].min[1] < QUARTER * REAL_MIN) ? (domain[ii].min[1]) : ((min[1] > domain[ii].min[1]) ? (min[1]) : (domain[ii].min[1]));
      sendBuf[overlapNum].zmin = (domain[ii].min[2] < QUARTER * REAL_MIN) ? (domain[ii].min[2]) : ((min[2] > domain[ii].min[2]) ? (min[2]) : (domain[ii].min[2]));
      //-------------------------------------------------------------------
      sendBuf[overlapNum].xmax = (domain[ii].max[0] > QUARTER * REAL_MAX) ? (domain[ii].max[0]) : ((max[0] < domain[ii].max[0]) ? (max[0]) : (domain[ii].max[0]));
      sendBuf[overlapNum].ymax = (domain[ii].max[1] > QUARTER * REAL_MAX) ? (domain[ii].max[1]) : ((max[1] < domain[ii].max[1]) ? (max[1]) : (domain[ii].max[1]));
      sendBuf[overlapNum].zmax = (domain[ii].max[2] > QUARTER * REAL_MAX) ? (domain[ii].max[2]) : ((max[2] < domain[ii].max[2]) ? (max[2]) : (domain[ii].max[2]));
      //-------------------------------------------------------------------
      overlapNum++;
      //-------------------------------------------------------------------
    }
      ...
      //-------------------------------------------------------------------
      chkMPIerr(MPI_Isend(&nullSend, 1, MPI_INT, ii, root.rank, root.comm, &(domain[ii].req)));
      //-------------------------------------------------------------------
    }/* else{ */
    //---------------------------------------------------------------------
  ...
  for(int ii = 0; ii < numOld; ii++){
    ...
    if( !find ){
#if 1
      fprintf(stderr, "numOld = %d, numProcs = %d, overlapNum = %d @ rank %d\n", numOld, numProcs, overlapNum, root.rank);
      fprintf(stderr, "local: xmin = %f, xmax = %f, ymin = %f, ymax = %f, zmin = %f, zmax = %f @ rank %d\n",
	      min[0], max[0], min[1], max[1], min[2], max[2], root.rank);
      for(int jj = 0; jj < numProcs; jj++){
	fprintf(stderr, "domain[%d]: xmin = %f, xmax = %f, ymin = %f, ymax = %f, zmin = %f, zmax = %f @ rank %d\n",
		jj, domain[jj].min[0], domain[jj].max[0], domain[jj].min[1], domain[jj].max[1], domain[jj].min[2], domain[jj].max[2], root.rank);
      }
      for(int jj = 0; jj < overlapNum; jj++){
	fprintf(stderr, "sendBuf[%d]: xmin = %f, xmax = %f, ymin = %f, ymax = %f, zmin = %f, zmax = %f @ rank %d\n",
		jj, sendBuf[jj].xmin, sendBuf[jj].xmax, sendBuf[jj].ymin, sendBuf[jj].ymax, sendBuf[jj].zmin, sendBuf[jj].zmax, root.rank);
      }
      fprintf(stderr, "ii = %d: x = %f, y = %f, z = %f @ rank %d\n", ii, src.pos[ii].x, src.pos[ii].y, src.pos[ii].z, root.rank);
#endif
  ...
  for(int ii = 0; ii < overlapNum; ii++){
    chkMPIerr(MPI_Isend(&(sendBuf[ii].num), 1, MPI_INT, sendBuf[ii].rank, root.rank, root.comm, &(domain[sendBuf[ii].rank].req)));
    if( ii > 0 )
  ...
  for(int ii = 0; ii < overlapNum; ii++){
    //---------------------------------------------------------------------
    chkMPIerr(MPI_Isend(&(dst.pos [sendBuf[ii].head]), sendBuf[ii].num, mpi.ipos         , sendBuf[ii].rank, root.rank, root.comm, &(sendBuf[ii].pos)));
#ifdef  GADGET_MAC
    chkMPIerr(MPI_Isend(&(dst.acc [sendBuf[ii].head]), sendBuf[ii].num, mpi.iacc         , sendBuf[ii].rank, root.rank, root.comm, &(sendBuf[ii].acc)));
#endif//GADGET_MAC
#ifdef  BLOCK_TIME_STEP
    chkMPIerr(MPI_Isend(&(dst.vel [sendBuf[ii].head]), sendBuf[ii].num, mpi.ivel         , sendBuf[ii].rank, root.rank, root.comm, &(sendBuf[ii].vel)));
    chkMPIerr(MPI_Isend(&(dst.time[sendBuf[ii].head]), sendBuf[ii].num, mpi.time         , sendBuf[ii].rank, root.rank, root.comm, &(sendBuf[ii].time)));
#else///BLOCK_TIME_STEP
    chkMPIerr(MPI_Isend(&(dst.vx  [sendBuf[ii].head]), sendBuf[ii].num, MPI_REALDAT      , sendBuf[ii].rank, root.rank, root.comm, &(sendBuf[ii].vx )));
    chkMPIerr(MPI_Isend(&(dst.vy  [sendBuf[ii].head]), sendBuf[ii].num, MPI_REALDAT      , sendBuf[ii].rank, root.rank, root.comm, &(sendBuf[ii].vy )));
    chkMPIerr(MPI_Isend(&(dst.vz  [sendBuf[ii].head]), sendBuf[ii].num, MPI_REALDAT      , sendBuf[ii].rank, root.rank, root.comm, &(sendBuf[ii].vz )));
#endif//BLOCK_TIME_STEP
    chkMPIerr(MPI_Isend(&(dst.idx [sendBuf[ii].head]), sendBuf[ii].num, MPI_UNSIGNED_LONG, sendBuf[ii].rank, root.rank, root.comm, &(sendBuf[ii].idx)));
    //---------------------------------------------------------------------
  }/* for(int ii = 0; ii < overlapNum; ii++){ */
  //-----------------------------------------------------------------------
  ...
  *numNew = 0;
  ...
  for(int ii = 0; ii < numProcs; ii++){
    ...
    if( recvBuf[ii].num != 0 ){
      //-------------------------------------------------------------------
      chkMPIerr(MPI_Irecv(&(src.pos [recvBuf[ii].head]), recvBuf[ii].num, mpi.ipos         , ii, ii, root.comm, &(recvBuf[ii].pos)));
#ifdef  GADGET_MAC
      chkMPIerr(MPI_Irecv(&(src.acc [recvBuf[ii].head]), recvBuf[ii].num, mpi.iacc         , ii, ii, root.comm, &(recvBuf[ii].acc)));
#endif//GADGET_MAC
#ifdef  BLOCK_TIME_STEP
      chkMPIerr(MPI_Irecv(&(src.vel [recvBuf[ii].head]), recvBuf[ii].num, mpi.ipos         , ii, ii, root.comm, &(recvBuf[ii].vel)));
      chkMPIerr(MPI_Irecv(&(src.time[recvBuf[ii].head]), recvBuf[ii].num, mpi.ipos         , ii, ii, root.comm, &(recvBuf[ii].time)));
#else///BLOCK_TIME_STEP
      chkMPIerr(MPI_Irecv(&(src.vx  [recvBuf[ii].head]), recvBuf[ii].num, MPI_REALDAT      , ii, ii, root.comm, &(recvBuf[ii].vx )));
      chkMPIerr(MPI_Irecv(&(src.vy  [recvBuf[ii].head]), recvBuf[ii].num, MPI_REALDAT      , ii, ii, root.comm, &(recvBuf[ii].vy )));
      chkMPIerr(MPI_Irecv(&(src.vz  [recvBuf[ii].head]), recvBuf[ii].num, MPI_REALDAT      , ii, ii, root.comm, &(recvBuf[ii].vz )));
#endif//BLOCK_TIME_STEP
      chkMPIerr(MPI_Irecv(&(src.idx [recvBuf[ii].head]), recvBuf[ii].num, MPI_UNSIGNED_LONG, ii, ii, root.comm, &(recvBuf[ii].idx)));
      //-------------------------------------------------------------------
  ...
  int diff_sum;
  chkMPIerr(MPI_Reduce(&diff, &diff_sum, 1, MPI_INT, MPI_SUM, 0, root.comm));
  if( root.rank == 0 )
    if( diff_sum != 0 ){
