...
#include "../util/scan_vec3_inc.cuh"


#ifndef SCAN_VEC3_INC_CU_MULTI_CALL
#define SCAN_VEC3_INC_CU_MULTI_CALL
...
__device__ __forceinline__ Type prefixSumVec3Warp(Type val, const int lane, volatile Type * smem, const int tidx)
{
  Type tmp;
 ...
 */
template <typename Type>
__device__ __forceinline__ Type totalSumVec3Warp(Type val, volatile Type * smem, const int tidx, const int head)
{
  Type tmp;
...
#endif//SCAN_VEC3_INC_CU_MULTI_CALL


/**
 * @fn PREFIX_SUM_VEC3_BLCK
 *
 * @brief Get parallel (inclusive) prefix sum within a block.
 */
template <typename Type>
__device__ __forceinline__ Type PREFIX_SUM_VEC3_BLCK(Type val, volatile Type * __restrict__ smem, const int lane, const int tidx)
{
  /** 1. prefix sum within warp */
  Type scan = prefixSumVec3Warp(val, lane, smem, tidx);


  /** 2. prefix sum about tail of each warp */
#   if  !defined(ENABLE_IMPLICIT_SYNC_WITHIN_WARP) && (NTHREADS_SCAN_VEC3_INC > 32)
  thread_block_tile<NTHREADS_SCAN_VEC3_INC >> 5> tile = tiled_partition<NTHREADS_SCAN_VEC3_INC >> 5>(this_thread_block());
#endif//!defined(ENABLE_IMPLICIT_SYNC_WITHIN_WARP) && (NTHREADS_SCAN_VEC3_INC > 32)
  __syncthreads();

  /** warpSize = 32 = 2^5; NTHREADS_SCAN_VEC3_INC <= 1024 --> NTHREADS_SCAN_VEC3_INC >> 5 <= 32 = warpSize */
  if( tidx < (NTHREADS_SCAN_VEC3_INC >> 5) ){
    val = ldvec(smem[tidx * warpSize + warpSize - 1]);

    stvec((Type *)smem, tidx, val);
#   if  (NTHREADS_SCAN_VEC3_INC >> 5) >=  2
    Type tmp;
    if( lane >=  1 ){      tmp = ldvec(smem[tidx -  1]);      val.x += tmp.x;      val.y += tmp.y;      val.z += tmp.z;      stvec((Type *)smem, tidx, val);    }
#ifndef ENABLE_IMPLICIT_SYNC_WITHIN_WARP
    tile.sync();
#endif//ENABLE_IMPLICIT_SYNC_WITHIN_WARP
#   if  (NTHREADS_SCAN_VEC3_INC >> 5) >=  4
    if( lane >=  2 ){      tmp = ldvec(smem[tidx -  2]);      val.x += tmp.x;      val.y += tmp.y;      val.z += tmp.z;      stvec((Type *)smem, tidx, val);    }
#ifndef ENABLE_IMPLICIT_SYNC_WITHIN_WARP
    tile.sync();
#endif//ENABLE_IMPLICIT_SYNC_WITHIN_WARP
#   if  (NTHREADS_SCAN_VEC3_INC >> 5) >=  8
    if( lane >=  4 ){      tmp = ldvec(smem[tidx -  4]);      val.x += tmp.x;      val.y += tmp.y;      val.z += tmp.z;      stvec((Type *)smem, tidx, val);    }
#ifndef ENABLE_IMPLICIT_SYNC_WITHIN_WARP
    tile.sync();
#endif//ENABLE_IMPLICIT_SYNC_WITHIN_WARP
#   if  (NTHREADS_SCAN_VEC3_INC >> 5) >= 16
    if( lane >=  8 ){      tmp = ldvec(smem[tidx -  8]);      val.x += tmp.x;      val.y += tmp.y;      val.z += tmp.z;      stvec((Type *)smem, tidx, val);    }
#ifndef ENABLE_IMPLICIT_SYNC_WITHIN_WARP
    tile.sync();
#endif//ENABLE_IMPLICIT_SYNC_WITHIN_WARP
#   if  (NTHREADS_SCAN_VEC3_INC >> 5) == 32
    if( lane >= 16 ){      tmp = ldvec(smem[tidx - 16]);      val.x += tmp.x;      val.y += tmp.y;      val.z += tmp.z;      stvec((Type *)smem, tidx, val);    }
#endif//(NTHREADS_SCAN_VEC3_INC >> 5) == 32
#endif//(NTHREADS_SCAN_VEC3_INC >> 5) >= 16
#endif//(NTHREADS_SCAN_VEC3_INC >> 5) >=  8
#endif//(NTHREADS_SCAN_VEC3_INC >> 5) >=  4
#endif//(NTHREADS_SCAN_VEC3_INC >> 5) >=  2
  ...
  if( tidx >= warpSize ){
    ...
    scan.x += tmp.x;    scan.y += tmp.y;    scan.z += tmp.z;
  }/* if( tidx >= warpSize ){ */
  __syncthreads();
 ...
 */
template <typename Type>
__device__ __forceinline__ Type PREFIX_SUM_VEC3_GRID
 ...
 volatile Type * __restrict__ gmem, const int bidx, const int bnum, int * __restrict__ gsync0, int * __restrict__ gsync1)
{
  ...
  *scanNum = ldvec(smem[NTHREADS_SCAN_VEC3_INC - 1]);
  ...
  if( bnum > 1 ){
    ...
    if( bidx == 0 ){
      const int Nloop = BLOCKSIZE(bnum, NTHREADS_SCAN_VEC3_INC);
      Type head = zero;
      for(int loop = 0; loop < Nloop; loop++){
	const int target = tidx + loop * NTHREADS_SCAN_VEC3_INC;

	/** load from the global memory */
	Type pidx = ((target < bnum) ? ldvec(gmem[target]) : zero);

	/** calculate local prefix sum */
	PREFIX_SUM_VEC3_BLCK(pidx, smem, lane, tidx);

	/** store to the global memory */
	if( target < bnum ){
	  Type tmp = ldvec(smem[tidx]);
	  tmp.x += head.x;	  tmp.y += head.y;	  tmp.z += head.z;
	  stvec((Type *)gmem, target, tmp);
	}/* if( target < bnum ){ */

	head.x += smem[NTHREADS_SCAN_VEC3_INC - 1].x;
	head.y += smem[NTHREADS_SCAN_VEC3_INC - 1].y;
	head.z += smem[NTHREADS_SCAN_VEC3_INC - 1].z;
	if( loop != (Nloop - 1) )
	  __syncthreads();
      }/* for(int loop = 0; loop < Nloop; loop++){ */
    }/* if( bidx == 0 ){ */
    ...
    scan.x += psub.x;    scan.y += psub.y;    scan.z += psub.z;
    psub = ldvec(smem[1]);
    scanNum->x = psub.x;    scanNum->y = psub.y;    scanNum->z = psub.z;
    __syncthreads();
...
/**
 * @fn PREFIX_SUM_VEC3_GRID_WITH_PARTITION
 *
 * @brief Get parallel (inclusive) prefix sum within a grid.
 *
 ...
 */
template <typename Type>
__device__ __forceinline__ Type PREFIX_SUM_VEC3_GRID_WITH_PARTITION
(Type val, volatile Type * __restrict__ smem, const int lane, const int tidx,
...
/**
 * @fn TOTAL_SUM_VEC3_BLCK
 *
 * @brief Get total sum within a block.
 */
template <typename Type>
__device__ __forceinline__ Type TOTAL_SUM_VEC3_BLCK(Type val, volatile Type * __restrict__ smem, const int tidx, const int head)
{
  /** 1. total sum within a warp */
  val = totalSumVec3Warp(val, smem, tidx, head);
...
/**
 * @fn TOTAL_SUM_VEC3_GRID
 *
 * @brief Get total sum within a grid.
 */
template <typename Type>
__device__ __forceinline__ Type TOTAL_SUM_VEC3_GRID
(Type val, volatile Type * __restrict__ smem, const int tidx, const int head,
 volatile Type * __restrict__ gmem, const int bidx, const int bnum, int * __restrict__ gsync0, int * __restrict__ gsync1)
{
  val = TOTAL_SUM_VEC3_BLCK(val, smem, tidx, head);
  ...
  if( bnum > 1 ){
	...
	subset = TOTAL_SUM_VEC3_BLCK(subset, smem, tidx, head);
	sum.x += subset.x;	sum.y += subset.y;	sum.z += subset.z;
