/*************************************************************************\
 *                                                                       *
                  last updated on 2016/03/07(Mon) 13:54:30
 *                                                                       *
...
/* #define REGISTERS_PER_THREAD_PH (38) */
/* #endif//GPUVER == 35 */
/* calcPHkey_kernel uses 38 registers @ GTX 750 Ti */
/* #   if  GPUVER == 50 */
/* #undef  REGISTERS_PER_THREAD_PH */
/* #define REGISTERS_PER_THREAD_PH (38) */
/* #endif//GPUVER == 50 */
/* calcPHkey_kernel uses 38 registers @ GTX 970 */
/* #   if  GPUVER == 52 */
/* #undef  REGISTERS_PER_THREAD_PH */
/* #define REGISTERS_PER_THREAD_PH (38) */
/* #endif//GPUVER == 52 */
...
/* #endif//NDEBUG */
  ...
  if( bidx == 0 ){
	...
	tmp = smem[tidx ^ 16];  if( tmp.x < min.x )    min.x = tmp.x;  if( tmp.y < min.y )    min.y = tmp.y;  if( tmp.z < min.z )    min.z = tmp.z;  smem[tidx] = min;
      ...
      //-------------------------------------------------------------------
      /* get maximum */
      /* devProp.numSM <= 16 in current GPUs */
#   if  (16 * NBLOCKS_PER_SM_PH) <= (NTHREADS_PH >> 1)
      smem[uidx] = max;
      if( bnum >  1 ){
	tmp = smem[uidx ^  1];  if( tmp.x > max.x )    max.x = tmp.x;  if( tmp.y > max.y )    max.y = tmp.y;  if( tmp.z > max.z )    max.z = tmp.z;  smem[uidx] = max;
      if( bnum >  2 ){
...
#else///(16 * NBLOCKS_PER_SM_PH) <= (NTHREADS_PH >> 1)
      __syncthreads();
      smem[tidx] = max;
      if( bnum >  1 ){
	tmp = smem[tidx ^  1];  if( tmp.x > max.x )    max.x = tmp.x;  if( tmp.y > max.y )    max.y = tmp.y;  if( tmp.z > max.z )    max.z = tmp.z;  smem[tidx] = max;
...
#endif//(16 * NBLOCKS_PER_SM_PH) <= (NTHREADS_PH >> 1)
      //-----------------------------------------------------------------
#if 0
      if( tidx == hidx )
	printf("%d-th warp:\t[%e:%e], [%e:%e], [%e:%e]\n", tidx >> 5, min.x, max.x, min.y, max.y, min.z, max.z);
#endif
      //-------------------------------------------------------------------
    }/* if( tidx < bnum  ){ */
    //---------------------------------------------------------------------
...
#   if  (16 * NBLOCKS_PER_SM_PH) <= (NTHREADS_PH >> 1)
	...
	smem[(NTHREADS_PH >> 1) + (tidx >> 5)] = max;/* := (NTHREADS_PH >> 1) + (tidx / warpSize) */
	//-----------------------------------------------------------------
      }/* if( (hidx == tidx) && (tidx < bnum) ){ */
      __syncthreads();
      //-------------------------------------------------------------------
 ...
 const deviceProp devProp)
{
  ...
  *info = (PHinfo *)malloc(NUM_PHKEY_LEVEL * sizeof(PHinfo));  if( *info == NULL ){    __KILL__(stderr, "ERROR: failure to allocate info");  }
  ...
  //-----------------------------------------------------------------------
  /* if( (devProp.numSM * NBLOCKS_PER_SM_PH) > (NTHREADS_PH >> 1) ){ */
  /*   __KILL__(stderr, "ERROR: product (%d) of devProp.numSM(%d) * NBLOCKS_PER_SM_PH(%d) must be smaller than half of NTHREADS_PH(%d) to use shared memory efficiently.\n", devProp.numSM * NBLOCKS_PER_SM_PH, devProp.numSM, NBLOCKS_PER_SM_PH, NTHREADS_PH); */
  /* } */
  //-----------------------------------------------------------------------
  ...
  //-----------------------------------------------------------------------
    ...
    /* devProp.numSM <= 16 is assumed, this is correct in current GPUs */
    if( devProp.numSM > 16 ){
      __KILL__(stderr, "ERROR: assumption that devProp.numSM(%d) <= 16 is broken; this is employed @ line %d and %d\n", devProp.numSM, 332, 379);
    }/* if( devProp.numSM > 16 ){ */
  }/* else{ */
  //-----------------------------------------------------------------------
