...
#   if  SMEM_SIZE_CALC_ACC > MAX_STATIC_SMEM_SIZE_BLOCK
  ...
  uint_real* smem = (uint_real*)&queue[NTHREADS * NQUEUE];
#ifdef  INDIVIDUAL_GRAVITATIONAL_SOFTENING
  real* eps2 = (real*)&smem[NTHREADS];
#endif//INDIVIDUAL_GRAVITATIONAL_SOFTENING
#else///USE_WARP_SHUFFLE_FUNC
#ifdef  INDIVIDUAL_GRAVITATIONAL_SOFTENING
...
#else///ADOPT_MIKI2013_PERFORMANCE_MODEL
      *reduce = share / block;
#endif//ADOPT_MIKI2013_PERFORMANCE_MODEL
#endif//BLOCK_TIME_STEP
...
__global__ void set_loss_cone_kernel(READ_ONLY int * RESTRICT list, READ_ONLY position * RESTRICT ipos, real * RESTRICT lmax2, const real lightspeed)
{
  const int tidx = THREADIDX_X1D;

  const position pi = ipos[list[tidx]];
