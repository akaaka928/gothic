/**
 * @file make.h
 *
 * @brief Header file for constructing octree structure
 *
 * @author Yohei Miki (University of Tokyo)
 * @author Masayuki Umemura (University of Tsukuba)
 *
 * @date 2017/10/26 (Thu)
 *
 * Copyright (C) 2017 Yohei Miki and Masayuki Umemura
 * All rights reserved.
 *
 * The MIT License is applied to this software, see LICENSE.txt
 *
 */
#ifndef MAKE_H
#define MAKE_H


#include <stdbool.h>

#include "macro.h"

#include "../misc/benchmark.h"
#include "../misc/structure.h"

#include "../sort/peano.h"
#include "../tree/macutil.h"


/**
 * @def SPLIT_CHILD_CELL
 *
 * @brief enables splitting child cell ...
 */
#define SPLIT_CHILD_CELL


/**
 * @def INDIVIDUAL_GRAVITATIONAL_SOFTENING
 *
 * @brief introduce individual particle softening
 */
/* #define INDIVIDUAL_GRAVITATIONAL_SOFTENING */


/**
 * @def TREE_SAFETY_VAL
 *
 * @brief a parameter for allocating memory for tree cells
 */
/* #define TREE_SAFETY_VAL (2.0f) */
/* #define TREE_SAFETY_VAL (1.5f) */
#define TREE_SAFETY_VAL (1.0f)


#define NUM_CELL_MAX ((int)ceilf((float)NUM_BODY_MAX *         TREE_SAFETY_VAL ))
#define NUM_NODE_MAX ((int)ceilf((float)NUM_BODY_MAX * (1.0f + TREE_SAFETY_VAL)))


/**
 * @def NLEAF
 *
 * @brief maximum number of leaf cells per tree cell
 */
#ifndef NLEAF
/* #define NLEAF (16) */
#define NLEAF (64)
/* #define NLEAF (128) */
#endif//NLEAF

/** NLEAF =   8: 0x1fffffff is 536870911 = 512M */
/** NLEAF =  16: 0x0fffffff is 268435455 = 256M */
/** NLEAF =  32: 0x07ffffff is 134217727 = 128M */
/** NLEAF =  64: 0x03ffffff is  67108863 =  64M */
/** NLEAF = 128: 0x01ffffff is  33554431 =  32M */
/** NLEAF = 256: 0x00ffffff is  16777215 =  16M */

/** minimum of NLEAF is set to be 8 */
/** (32 - IDXBITS) contains # of child pseudo-particles of the corresponding pseudo particle (1, 2, 3, ..., NLEAF) */
/** MEMO: in case of NLEAF = 8, then 000 -> 1, 001 -> 2, 010 -> 3, 011 -> 4, 100 -> 5, 101 -> 6, 110 -> 7, 111 -> 8 */
#   if  NLEAF <=  8
#undef  NLEAF
#define NLEAF    (8)
#define IDXBITS (29)
#define IDXMASK (0x1fffffff)
#else///NLEAF <=  8
#   if  NLEAF <= 16
#define IDXBITS (28)
#define IDXMASK (0xfffffff)
#else///NLEAF <= 16
#   if  NLEAF <= 32
#define IDXBITS (27)
#define IDXMASK (0x7ffffff)
#else///NLEAF <= 32
#   if  NLEAF <= 64
#define IDXBITS (26)
#define IDXMASK (0x3ffffff)
#else///NLEAF <= 64
#   if  NLEAF <= 128
#define IDXBITS (25)
#define IDXMASK (0x1ffffff)
#else///NLEAF <= 128
#undef  NLEAF
#define NLEAF   (256)
#define IDXBITS (24)
#define IDXMASK (0xffffff)
#endif//NLEAF <= 128
#endif//NLEAF <= 64
#endif//NLEAF <= 32
#endif//NLEAF <= 16
#endif//NLEAF <=  8

#define NULL_CELL (IDXMASK - 1)
#define NULL_NODE (IDXMASK)


/**
 * @def NCRIT
 *
 * @brief critical number of child cells to terminate tree construction
 */
#ifndef  NCRIT
#define  NCRIT (8)
#endif///NCRIT

#   if  NCRIT > NLEAF
#undef  NCRIT
#define NCRIT   NLEAF
#endif//NCRIT > NLEAF


#define CELL_UNIT (8)


#define NUM_ALLOC_TREE_CELL ((((IDXMASK) + 1) < (NUM_CELL_MAX)) ? ((IDXMASK) + 1) : (NUM_CELL_MAX))
#define NUM_ALLOC_TREE_NODE ((((IDXMASK) + 1) < (NUM_NODE_MAX)) ? ((IDXMASK) + 1) : (NUM_NODE_MAX))


#define NI_BMAX_ESTIMATE (32)
#define NJ_BMAX_ESTIMATE (NLEAF)

#   if  NJ_BMAX_ESTIMATE < NLEAF
#undef  NJ_BMAX_ESTIMATE
#define NJ_BMAX_ESTIMATE   NLEAF
#endif//NJ_BMAX_ESTIMATE < NLEAF


#define NUM_ALLOC_MACBUF (16384)


/**
 * @struct treecell
 *
 * @brief structure for tree cell
 */
typedef struct __align__(8)
{
  int num, head;/**< the number of N-body particles and the index of the head particle */
} treecell;

static const treecell null_cell = {0, NULL_CELL};


/**
 * @struct jparticle
 *
 * @brief structure for j-particles
 */
typedef struct __align__(16)
{
  real y, x, w, z;
} jparticle;


#ifdef  INDIVIDUAL_GRAVITATIONAL_SOFTENING
/**
 * @struct jmass
 *
 * @brief structure for mass and softening parameter of j-particles
 */
typedef struct __align__(8)
{
  real mass, eps2;
} jmass;
#else///INDIVIDUAL_GRAVITATIONAL_SOFTENING
/**
 * @typedef jmass
 *
 * @brief real
 */
typedef real jmass;
#endif//INDIVIDUAL_GRAVITATIONAL_SOFTENING


/**
 * @struct laneinfo
 *
 * @brief structure for information on lane
 */
typedef struct __align__(8)
{
  int head, num;
} laneinfo;


/**
 * @struct soaTreeCell
 *
 * @brief structure for tree cell (SoA)
 */
typedef struct
{
  PHinfo *level;
  treecell *cell;
  bool *leaf;
  uint *ptag;
  PHint *hkey;
  uint *parent, *children;
} soaTreeCell;

/**
 * @struct soaTreeNode
 *
 * @brief structure for tree node (SoA)
 */
typedef struct
{
  uint *more;
  int *jtag;
  int *node2cell;
  jparticle *jpos;
  jmass *mj;
  real *bmax;
#ifdef  WS93_MAC
  real *mr2;
#endif//WS93_MAC
#ifdef  GADGET_MAC
  real *mac;
#endif//GADGET_MAC
} soaTreeNode;


#endif//MAKE_H
