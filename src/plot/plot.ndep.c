/*************************************************************************\
 *                                                                       *
                  last updated on 2016/05/26(Thu) 16:35:51
 *                                                                       *
...
int main(int argc, char **argv)
{
  ...
  /* prepare dataset */
  //-----------------------------------------------------------------------
  /* /\* read the elapsed time (Ntot dependence) *\/ */
  /* static PLINT ntot[NARC][NDST][NNUM]; */
  /* static PLFLT ttot[NARC][NDST][NNUM], mean[NARC][NDST][NNUM]; */
  /* for(int ii = 0; ii < NARC; ii++) */
  /*   for(int jj = 0; jj < NDST; jj++){ */
  /*     //------------------------------------------------------------------- */
  /*     FILE *fp; */
  /*     char filename[128]; */
  /*     sprintf(filename, "%s/%s.%s.log", NTOTFOLDER, icname[jj], arcname[ii]); */
  /*     fp = fopen(filename, "r"); */
  /*     if( fp == NULL ){	__KILL__(stderr, "ERROR: failure to open \"%s\"\n", filename);      } */
  /*     int ll = 0; */
  /*     int step; */
  /*     while( EOF != fscanf(fp, "%d\t%*e\t%le\t%d\t%le", &ntot[ii][jj][ll], &ttot[ii][jj][ll], &step, &mean[ii][jj][ll]) ){ */
  /* 	if( step != 1024 ) */
  /* 	  mean[ii][jj][ll] = DBL_MAX; */
  /* 	ll++; */
  /*     }/\* while( EOF != fscanf(fp, "%d\t%*e\t%le\t%d\t%le", &ntot[ii][jj][ll], &ttot[ii][jj][ll], &step, &mean[ii][jj][ll]) ){ *\/ */
  /*     fclose(fp); */
  /*     //------------------------------------------------------------------- */
  /*     for(int kk = 0; kk < ll; kk++){ */
  /* 	ttot[ii][jj][kk] = log10(ttot[ii][jj][kk]); */
  /* 	if( mean[ii][jj][kk] < DBL_MAX ) */
  /* 	  mean[ii][jj][kk] = log10(mean[ii][jj][kk]); */
  /*     }/\* for(int kk = 0; kk < ll; kk++){ *\/ */
  /*     //------------------------------------------------------------------- */
  /*     for(int mm = ll; mm < NNUM; mm++){ */
  /* 	ntot[ii][jj][mm] = ntot[ii][jj][ll - 1]; */
  /* 	/\* ttot[ii][jj][mm] = ttot[ii][jj][ll - 1]; *\/ */
  /* 	/\* mean[ii][jj][mm] = mean[ii][jj][ll - 1]; *\/ */
  /* 	ttot[ii][jj][mm] = DBL_MAX; */
  /* 	mean[ii][jj][mm] = DBL_MAX; */
  /*     }/\* for(int mm = ll; mm < NNUM; mm++){ *\/ */
  /*     //------------------------------------------------------------------- */
  /*   }/\* for(int jj = 0; jj < NDST; jj++){ *\/ */
  //-----------------------------------------------------------------------
...
/* #endif//LOGPLOT_VER */
/*   //----------------------------------------------------------------------- */
/*   plotNtotDependence(ntot, ttot, range, "ttot", false, argc, argv); */
  //-----------------------------------------------------------------------
#ifdef  LOGPLOT_VER
