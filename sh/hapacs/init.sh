...
###############################################################
# for mvapich2
mpirun -n $PROCS -hostfile $PBS_NODEFILE -l $EXEC -file=$FILE -config=$CONFIG -Ntot=$NTOT -eps=$EPS -ft=$FINISH -eta=$ETA -snapshotInterval=$INTERVAL -saveInterval=$SAVE 1>>$STDOUT 2>>$STDERR
###############################################################
TIME=`date`
